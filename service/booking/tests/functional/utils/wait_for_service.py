...
service_port: int = int(os.getenv('SERVICE_PORT', 4800))
...
def wait_service():
    try:
        response = requests.get(
            f'http://{service_host}:{service_port}/messages/api/ping',
...
if __name__ == '__main__':
    ...
    try:
        ...
        logger.error(e)
        logger.debug(f'http://{service_host}:{service_port}/messages/api/ping')
